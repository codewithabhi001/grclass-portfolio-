      ...
      {loading ? (
        ...
        <>
          ...
          <section className="container-page section">
            <Link href="/news" className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider text-secondary hover:text-accent">
              <ArrowLeft className="h-3.5 w-3.5" /> All articles
            </Link>
              <img
                src={typeof item.thumbnail_url === "string" ? item.thumbnail_url : (item.thumbnail_url as any).src || newsHero.src}
                alt={item.title}
                className="mt-8 aspect-[16/9] w-full object-cover"
                onError={(e) => {
                  (e.target as HTMLImageElement).src = newsHero.src;
                }}
              />
            ...
            {item.body_html ? (
              <div
                className="dynamic-html-content mt-10 max-w-3xl text-body-lg font-light leading-[1.85] text-muted-foreground"
                dangerouslySetInnerHTML={{ __html: item.body_html }}
              />
            ) : (
              <article className="prose-editorial mt-10 max-w-3xl space-y-5 text-body-lg font-light leading-[1.85] text-muted-foreground">
                <p className="text-title font-normal text-foreground">{item.excerpt}</p>
                <p>
                ...
                <ul className="space-y-2 text-body text-foreground/85">
          ...
          <section className="border-t border-border bg-secondary-soft section">
            <div className="container-page">
              ...
              <div className="mt-8 grid gap-px bg-border md:grid-cols-3">
                {related.map((n) => (
                  <Link key={n.slug} href={`/news/${n.slug}`} className="bg-background p-7 transition-colors hover:bg-card">
                    <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent">{n.category}</span>
                    <h3 className="h-display mt-4 text-body-lg text-primary">{n.title}</h3>
                  </Link>
                ))}
              </div>
            </div>
